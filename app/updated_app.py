...
with st.sidebar:
    ...
    with st.expander("Setup"):
        ...
        if st.session_state["setup_prompt"] == "Custom":
            st.session_state["setup_prompt"] = st.text_input(
                "System message", value="You are a helpful assistant.")

        st.write(
            'Tip: You can change the setup prompt on the fly for generating more creative responses.')

        st.session_state["use_cutoff_date"] = st.checkbox(
            "Include knowlegde cutoff", value=False, help="Include the cutoff date in the setup prompt. This is useful for limiting the AI's knowledge to a certain date.")
        if st.session_state["use_cutoff_date"]:
            max_cutoff_date = datetime.date(2021, 9, 1)
            st.session_state["date_cutoff"] = st.date_input(
                "Cutoff date", value=max_cutoff_date, help="This is the date of the knowledge cutoff. The AI will not be able to provide detailed information about events that happened after this date.", max_value=max_cutoff_date)
            st.session_state["date_cutoff_today"] = st.date_input("Today's date", value=datetime.date.today(
            ), help="The date of today.", max_value=datetime.date.today())

            st.session_state["setup_prompt"] += f" {get_cutoff_string()}"

    with st.expander("Chat options"):
        st.session_state["streaming_output"] = st.checkbox(
            "Streaming output", value=True,  help="Show the output as if the AI is typing it out, instead of all at once.")
        clear_chat = st.button("Clear chat", help="Clear the chat history",
                               use_container_width=True, on_click=clear_chat_history)
    ...
    with st.expander("API Configuration"):
        st.text_input("API token", value=st.secrets["OPENAI_API_KEY"], type="password",
                      help="You can find your API token [here](https://beta.openai.com/account/api-keys).", disabled=True)
        ...
        button_generate_new_session_identifier = st.button(
...
with st.sidebar:
    ...
    st.write("Upload a file from your device.")
    uploaded_file = st.file_uploader("Choose a file")
    if uploaded_file is not None:
        st.write("File has been uploaded successfully!")
    
    st.write("\n\n**To upload a file from Google Drive:**")
    st.write("1. Download the file from Google Drive to your device.")
    st.write("2. Use the 'Choose a file' button above to upload the downloaded file.")
